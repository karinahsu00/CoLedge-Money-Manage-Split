...
function aggregateMembers(transactions) {
    const map = {};
    transactions.forEach(t => {
        // Fallback: if members[] is empty, use legacy member field, else default to "You"
        const members = Array.isArray(t.members) ? t.members.filter(Boolean) : [];
        const resolvedMembers = members.length > 0 ? members : [t.member || 'You'];

        const amount = parseFloat(t.amount) || 0;
        const share = resolvedMembers.length > 0 ? amount / resolvedMembers.length : 0;
...
const AnalyticsPage = () => {
    ...
    (async () => {
        try {
            ...
            // keep analytics usable even if accounts fail to load
            setAccounts([]);
        }
    })();
}, []);
    ...
    const rangedTransactions = filterByRange(
        transactions,
        rangeType, selectedYear, selectedMonth, startDate, endDate
    );

    const expenseTransactions = rangedTransactions.filter(t => t.type === 'expense');
    const incomeTransactions = rangedTransactions.filter(t => t.type === 'income');
    const transferTransactions = rangedTransactions.filter(t => t.type === 'transfer');

    const byCategory = aggregate(expenseTransactions, t => t.category);
    const byAccount = aggregate(expenseTransactions, t => accountNameById.get(t.accountId) || 'Unknown Account');
    ...
    const totalTransferAmount = transferTransactions.reduce(
        (s, t) => s + (parseFloat(t.amount) || 0),
        0
    );
