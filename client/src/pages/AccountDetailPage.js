    ...
    <div className="dashboard-container account-detail-page">
      ...
      <div className="dashboard-content">
        ...
        <div className="transaction-list">
          <h2>Transactions</h2>
          <div className="tx-table-wrap"><table className="tx-table"><thead><tr><th>Date</th><th style={{textAlign: 'right'}}>Amount</th></tr></thead><tbody>{transactions.map(t => (<tr key={t.id}><td>{t.date}</td><td style={{textAlign: 'right'}}>{(Number(t.amount) * rate).toFixed(2)} USD ({t.amount} {currency})</td></tr>))}</tbody></table></div>
          <div className="accounts-cards">{transactions.map(t => (<div key={t.id} className="account-card-mobile" style={{ background: 'white', padding: '15px', borderRadius: '10px', marginBottom: '10px' }}><div style={{ display: 'flex', justifyContent: 'space-between' }}><span>{t.date}</span><span style={{ fontWeight: 600 }}>{(Number(t.amount) * rate).toFixed(2)} USD</span></div></div>))}</div>
        </div>
      </div>
      <MobileTabBar />
    </div>
